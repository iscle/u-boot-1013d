...
static int dram_initial(void) {
	...
	setbits_le32(&dram->sctlr, 0x1);
	while ((readl(&dram->sctlr) & 0x1) && time--) {
		if (time == 0)
			return 0;
	}
	return 1;
...
static int dram_delay_scan(void) {
	...
	setbits_le32(&dram->ddlyr, 0x1);
	while ((readl(&dram->ddlyr) & 0x1) && time--) {
		if (time == 0)
			return 0;
	}
	return 1;
...
static int dram_para_setup(struct dram_para * para) {
	...
	writel(val, &dram->sconr);
	setbits_le32(&dram->sctlr, 0x1 << 19);
	return dram_initial();
}
...
static u32 dram_check_delay(u32 bwidth) {
	...
	for (i = 0; i < dsize; i++) {
		if (i == 0)
			dflag = readl(&dram->drptr0);
		else if (i == 1)
			dflag = readl(&dram->drptr1);
		else if (i == 2)
			dflag = readl(&dram->drptr2);
		else if (i == 3)
			dflag = readl(&dram->drptr3);
...
static u32 dram_scan_readpipe(struct dram_para * para) {
	...
	if (para->sdr_ddr == DRAM_TYPE_DDR) {
		for (i = 0; i < 8; i++) {
			...
			dram_delay_scan();
			readpipe[i] = 0;
			if ((((readl(&dram->ddlyr) >> 4) & 0x3) == 0x0) &&
			        (((readl(&dram->ddlyr) >> 4) & 0x1) == 0x0)) {
